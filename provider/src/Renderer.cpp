...
RenderMessage::RenderMessage(TileInfo &tile, ChartSet *set,
                             Renderer *renderer, long settingsSequence) : RenderMessageBase(tile, set, renderer, settingsSequence)
{
    cacheResult = NULL;
    renderResult = NULL;
}
...
FeatureInfoMessage::FeatureInfoMessage(
    ...
    float lat,
    float lon,
    float tolerance) : RenderMessageBase(tile, set, renderer, settingsSequence)
{
    this->lat = lat;
    this->lon = lon;
...
Renderer::RenderResult Renderer::renderTile(ChartSet *set, TileInfo &tile, CacheEntry *&out, long timeout, bool forCache)
{
    ...
    if (!PrepareRenderMessage(set, tile, msg))
        return RENDER_NOCHART;
...
bool objectDescriptionSort(ObjectDescription first, ObjectDescription second)
{
    ...
    if (first.IsPoint() && !second.IsPoint())
        return true;
    if (second.IsPoint() && !first.IsPoint())
        return false;
    if (first.distance < second.distance)
        return true;
    if (first.distance > second.distance)
        return false;
    if (strcmp("LIGHTS", first.featureName) == 0 && strcmp("LIGHTS", second.featureName) != 0)
        return true;
    return false;
}
...
wxString objectToHtml(ObjectDescription *obj)
{
    ...
    if (obj->IsPoint())
    {
        rt.Append("<div class=\"coordinate\">")
            .Append(StringHelper::safeHtmlString(formatCoordinate(obj->lat, true)))
            .Append(" ")
    ...
    double lat, double lon, double tolerance)
{
    ...
    if (!PrepareRenderMessage(set, tile, msg, true))
    ...
    for (it = completeList->begin(); it != completeList->end(); it++)
    {
        ...
        if (!it->IsPoint())
            continue; // leave out others for now
        if (first == NULL)
            first = &(*it);
        ...
        if (light != wxEmptyString)
            properties["light"].Append(light);
        for (mit = mappings.begin(); mit != mappings.end(); mit++)
        {
            if (!(*mit)->MatchesFeature(it->featureName))
                continue;
...
wxString Renderer::FeatureRequestApi(
    ChartSet *set,
    TileInfo &tile,
    double lat, double lon, double tolerance)
{
    static const wxString fct("Renderer::FeatureRequestApi");
    LOG_DEBUG(wxT("%s: set=%s, tile=%s"), fct, set->GetKey(), tile.ToString());
    ...
    wxString result = wxT("{ \"s57objects\": [");

    bool firstObj = true;
    for (auto &obj : *completeList)
    {
        if (!firstObj)
        {
            result.Append(wxT(","));
        ...
        wxString objStr = wxString::Format(wxT("{\"featureName\":\"%s\",\"name\":\"%s\",\"lat\":%f,\"lon\":%f,\"params\":{"),
                                           StringHelper::safeJsonString(obj.featureName), StringHelper::safeJsonString(obj.name), obj.lat, obj.lon);

        bool firstParam = true;
        for (const auto &param : obj.param)
        {
            if (!firstParam)
            {
                objStr.Append(wxT(","));
            ...
            firstParam = false;
            objStr.Append(wxString::Format(wxT("\"%s\":\"%s\""),
                                           StringHelper::safeJsonString(param.first),
                                           StringHelper::safeJsonString(param.second)));
        }
