...
public:
    ...
    virtual HTTPResponse *HandleRequest(HTTPRequest *request)
    {
        ...
        LOG_INFO(wxString::Format("ChartSet count: %d", (int)map->size()));

        for (it = map->begin(); it != map->end(); it++)
        {
            LOG_INFO(wxString::Format("Checking chart: %s", it->second->GetKey()));

            if (!it->second->IsActive())
            {
                LOG_INFO(wxString::Format("Chart %s is not active", it->second->GetKey()));
                continue;
            }

            if (!it->second->IsReady())
            {
                LOG_INFO(wxString::Format("Chart %s is not ready", it->second->GetKey()));
                continue;
            }

            if (it->second->DisabledByErrors())
            {
                LOG_INFO(wxString::Format("Chart %s disabled by errors", it->second->GetKey()));
                continue;
            }
